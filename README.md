# OS-Experiment
来自 : 操作系统课程实验 NUAA 给个小星星❤支持一下呗=w=
...
```
    程序首先定义计算精度，公式中有40000个数相加减。先初始化数列数组，将每一项需要运算的数计算出来。随后创建一个线程，运算
20001-40000项的数值，运算过程中调用子函数在主线程中运算1-20000项的数值，运算结束后等待另一个线程计算基数，将两个数相加乘
以4得到近似PI的值。输出子函数运算结果，辅助线程运算结果，相加运算结果以及近似值结果。
```
...
```
    与上一题类似，将主线程中的计算部分去除，平均分摊到子线程中，并且创建多个子线程，首先确定计算精度，随后确定每个子线程分
配的计算量。主函数控制所有子进程计算，最终等待所有子进程运算结束统一添加至结果值。
```
### sort.c: 多线程排序
