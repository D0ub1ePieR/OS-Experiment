# OS-Experiment
来自 : 操作系统课程实验 NUAA 给个小星星❤支持一下呗=w= <br>
...
```
    首先确定整个乱序数组和每个线程负责的排序范围。使用选择排序将前半部分在主进程中后半部分在辅助进程中进行排序，将无序数组
的前后两部分改变为两个顺序数组，随后在主线程中使用二路归并排序最终得到一个有序数组。
```
...
```
    设置两个大小相同，大小为生产内容一半的缓冲区，并设置两个互斥变量，两个等待满信号量和两个等待空信号量。生产者负责每一个
循环生产一个小写字母，如果一号缓冲区满了则等待别的进程发送一个等待空信号给生产者继续生产，生产满后发送一个满信号。计算者负
责从一号缓冲区拿去小写字母转换为大写字母放入二号缓冲区，等待一号缓冲区满信号随后从中拿去数据，并发送空信号，等待二号缓冲区
空信号，放入二号缓冲区，并发送满信号。消费者负责输出大写字母结果，等待二号缓冲区的满信号，取出数据后发送空信号。
```
### pc2.c: 使用信号量解决生产者、计算者、消费者问题
&emsp;&emsp;功能和前面的实验相同，使用信号量解决<br>
```
    将上一题中的各个信号转换为统一结构体表示信号量，来控制各个线程的互斥以及等待及发送信号。控制的等待信号量，自身值减一，
如果值小于零则等待信号接收，并产生互斥。控制的发送信号量，自身值加一，并发送信号。消费者依旧还是调用缓冲区2的互斥信号，并
在开始等待满信号，末尾发送空信号。对于计算者，依旧使用两个缓冲区的互斥信号，首先对缓冲区1取数等待满信号发送空信号，存入缓
冲区2等待空信号发送满信号。生产者对缓冲区1进行操作，不断产生新的字母等待空信号放入缓冲区1，并发送满信号。在初始化时，将满
信号的signal设置为0，互斥信号的signal设置为1，空信号的signal设置为1-n-1，也就是表示实际使用的缓冲区大小。
```
...
```
    主线程创建第一个子线程，线程接收变量为计数的变量，如果不为环末端线程，则将数加一，并创建传递到下一个线程中，如果遇到
环末端线程，则将值加一传入环首端线程中，这样会导致无限循环下去，不会停止程序，所以需要设定一个阈值，当计数器增加到那个数
的时候便会杀死所有进程回到主线程结束程序。
```
